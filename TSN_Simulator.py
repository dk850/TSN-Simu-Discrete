...
class ST(Packet):
    ...
    def __init__(self, source, destination, priority, delay_jitter_constraints, period, deadline, \
        ...
        super().__init__(source, destination, 1, name, offset)
...
class Sporadic_Hard(NonST):
    ...
    def __init__(self, source, destination, priority, minimal_inter_release_time, delay_jitter_constraints, \
        ...
        super().__init__(source, destination, 2, minimal_inter_release_time, delay_jitter_constraints, period, name, offset)
        self.hard_deadline = deadline
...
class Sporadic_Soft(NonST):
    ...
    def __init__(self, source, destination, priority, minimal_inter_release_time, delay_jitter_constraints, \
                 period, deadline, name="unnamed", offset="0"):
        super().__init__(source, destination, 3, minimal_inter_release_time, delay_jitter_constraints, period, name, offset)
...
class Best_Effort(NonST):
    ...
    def __init__(self, source, destination, priority, minimal_inter_release_time, delay_jitter_constraints, \
                 period, name="unnamed", offset="0"):
        super().__init__(source, destination, 4, minimal_inter_release_time, delay_jitter_constraints, period, name, offset)
...
def parse_network_topo(f_network_topo):
    ...
    if(len(g_node_id_dict) > MAX_NODE_COUNT):
        print("ERROR: Too many nodes (MAX:", str(MAX_NODE_COUNT)+")")
        return -1
...
def parse_routing_table(f_network_topo):
    ...
    #print("SWITCHES:", switch_list)
    #print("END STATIONS:", end_station_list)
    ...
    while 1:
        ...
        for switch_id in routing_table:  # for each switch in the routing table
            ...
            switches_to_delete = []  # prepare list of indexes that we are to delete upon completion of the scan

            for child_index in range(len(routing_table[switch_id])):  # check every child of the current switch
                if routing_table[switch_id][child_index][0] in switch_list:  # if the child of a switch is another switch we have found a hop switch
                    changes += 1  # increase change count
                    #print("Found a switch in children of switch:", switch_id)
                    #print("child switch is", routing_table[switch_id][child_index][0], "at pos", child_index)
                    #print("Switch pre-change:", routing_table[switch_id])

                    # insert hop entries from child switch
                    for node in routing_table[routing_table[switch_id][child_index][0]]:  # for each node connected to the child switch
                        #print(node, routing_table[switch_id][child_index])
                        ...
                        if(routing_table[switch_id][child_index][1] == switch_id):
                            part_out_tuple.append(routing_table[switch_id][child_index][0])  # insert hop switch_id
                        else:
                            part_out_tuple.append(routing_table[switch_id][child_index][1])  # insert hop switch_id
    ...
    for switch_id in routing_table:
        ...
        for child_id in routing_table[switch_id]:  # have to check each child of the switch
    ...
    for key in g_node_id_dict:
        if(key in routing_table):
            ...
            #print("switch:", key, "table set:", g_node_id_dict[key].routing_table)
